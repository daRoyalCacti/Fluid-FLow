...
constexpr unsigned max_its[no_solver_choices] = {1000, 5000, 10000};
constexpr unsigned dims[no_solver_choices]    = {20, 20, 25};
...
void solve_velocity(const boundary_conditions& BC, const big_matrix &A, const big_vec_v &b, big_vec_v &vc, big_vec_v &v_n, const double accuracy_percent) {
    for (unsigned i; i < no_solver_choices; i++) {
        //solve(Q, s, p_c, max_its[i], dims[i]);
        solve(A, b.xv, vc.xv, max_its[i], dims[i]);
        solve(A, b.yv, vc.yv, max_its[i], dims[i]);
        solve(A, b.zv, vc.zv, max_its[i], dims[i]);
