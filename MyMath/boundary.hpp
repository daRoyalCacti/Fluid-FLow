...
struct boundary_points {
    ...
    [[nodiscard]] constexpr inline bool has_2right(const unsigned i, const unsigned j, const unsigned k) const {
        if (i+1 > N) return false;
        return v[get_index(i,j,k)].has_right && v[get_index(i+1,j,k)].has_right;
    }
    [[nodiscard]] constexpr inline bool has_2down(const unsigned i, const unsigned j, const unsigned k) const {
        if (j-1 < 0) return false;
        return v[get_index(i,j,k)].has_down && v[get_index(i,j-1,k)].has_down;
    }
    [[nodiscard]] constexpr inline bool has_2up(const unsigned i, const unsigned j, const unsigned k) const {
        if (j+1 > M) return false;
        return v[get_index(i,j,k)].has_up && v[get_index(i,j+1,k)].has_up;
    }
    [[nodiscard]] constexpr inline bool has_2front(const unsigned i, const unsigned j, const unsigned k) const {
        if (k-1 < 0) return false;
        return v[get_index(i,j,k)].has_front && v[get_index(i,j,k-1)].has_front;
    }
    [[nodiscard]] constexpr inline bool has_2back(const unsigned i, const unsigned j, const unsigned k) const {
